...
test('should split string templates into parts', async () => {
  expect(await ƒ(`\"ar$('one')ou$('two')nd\" template-parts`)).toEqual(
    τ([
      {
        raw: ['ar', 'ou', 'nd'],
        strings: ['ar', 'ou', 'nd'],
  ...
  expect(await ƒ('"ar$(1)ou$(1 1 +)nd" template-parts')).toEqual(
    τ([
      {
        raw: ['ar', 'ou', 'nd'],
        strings: ['ar', 'ou', 'nd'],
  ...
  expect(await ƒ(`'\\u{41}$( 42 )b' template-parts`)).toEqual(
    τ([
      {
        raw: ['\\u{41}', 'b'],
        strings: ['A', 'b'],
...
test('should process string template with', async () => {
  expect(await ƒ('"-1 sqrt = $( -1 )" [ sqrt ] template-with')).toEqual(
    `[ '-1 sqrt = 0+1i' ]`
  );
  expect(await ƒ('"abc ucase = $( \'abc\' )" [ ucase ] template-with')).toEqual(
    `[ 'abc ucase = ABC' ]`
  );
});
