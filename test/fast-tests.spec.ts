...
describe('core', () => {
  test('drop', async () => {
    await fc.assert(fc.asyncProperty(value, async (v) => {
      expect(await ƒ`${v} drop`).toEqual(τ([]));
    }));
  });

  test('swap', async () => {
    await fc.assert(fc.asyncProperty(value, value, async (a, b) => {
...
describe('arrays', () => {
  test('ln', async () => {
    await fc.assert(fc.asyncProperty(array, async (v) => {
      expect(await ƒ`${v} ln`).toEqual(τ([v.length]));
    }));
  });

  test('join', async () => {
    await fc.assert(fc.asyncProperty(array, array, async (a, b) => {
  ...
  test('eq', async () => {
    await fc.assert(fc.asyncProperty(array, async (a) => {
...
describe('strings', () => {
  test('ln', async () => {
    await fc.assert(fc.asyncProperty(fc.hexa(), async (v) => {
      expect(await ƒ`${v} ln`).toEqual(τ([`${v}`.length]));
    }));
  });

  test('join', async () => {
    await fc.assert(fc.asyncProperty(fc.hexa(), fc.hexa(), async (a, b) => {
      expect(await ƒ`${a} ${b} +`).toEqual(τ([a + b]));
      expect(await ƒ`${a} ${b} >>`).toEqual(τ([a + b]));
      expect(await ƒ`${a} ${b} <<`).toEqual(τ([a + b]));
    }));
  ...
  test('eq', async () => {
    await fc.assert(fc.asyncProperty(fc.hexa(), async (a) => {
      expect(await ƒ`${a} ${a} =`).toEqual(τ([true]));
    }));
  });
  ...
  test('Identity', async () => {
    await fc.assert(fc.asyncProperty(fc.hexa(), async (n) => {
...
describe('math', () => {
  test('Identity', async () => {
    await fc.assert(fc.asyncProperty(fc.nat(), async (n) => {
...
describe('boolean logic', () => {
  test('Dominance', async () => {
    await fc.assert(fc.asyncProperty(fc.boolean(), async (a) => {
  ...
  test('Commutativity', async () => {
    await fc.assert(fc.asyncProperty(fc.boolean(), fc.boolean(), async (a, b) => {
...
test('integer arithmetic', async () => {
  await fc.assert(fc.asyncProperty(fc.integer(), async (a) => {
...
test('polynomial identities', async () => {
  await fc.assert(fc.asyncProperty(fc.integer(), fc.integer(), async (a, b) => {
...
test('infinity identities', async () => {
  ...
  await fc.assert(fc.asyncProperty(fc.nat().filter(n => n > 0), async (a) => {
    expect(await ƒ`${a} infinity *`).toEqual(`[ Infinity ]`);
    expect(await ƒ`${a} -infinity *`).toEqual(`[ -Infinity ]`);
