...
test('should create objects object', async () => {
  expect(
    await ƒ('[ "first" "Manfred" "last" "von Thun" ] object')
...
test('should create objects object, cont', async () => {
  ...
  expect(await ƒ('{ first: "Manfred", last: "von Thun" }')).toEqual(`[ { first: 'Manfred', last: 'von Thun' } ]`);
  expect(
    await ƒ('{ first: "Manfred", last: [ "von" "Thun" ] " " * }')
  ).toEqual(`[ { first: 'Manfred', last: 'von Thun' } ]`);
});
...
test('should get single values usint @', async () => {
  expect(
    await ƒ('{ first: "Manfred" last: "von Thun" } first: @')
  ).toEqual(`[ 'Manfred' ]`);
...
test('should join objects without mutations', async () => {
  ...
  expect(
    await ƒ('{ first: "Manfred" } dup { last: "von Thun" } >>')
  ).toEqual(`[ { first: 'Manfred' } { last: 'von Thun', first: 'Manfred' } ]`);
  expect(
    await ƒ('{ first: "Manfred" } dup { last: "von Thun" } <<')
  ).toEqual(`[ { first: 'Manfred' } { first: 'Manfred', last: 'von Thun' } ]`);
});
