import * as Decimal from 'decimal.js';

import { typed } from './typed';
import { g, c } from './gamma';

export const BigNumber = Decimal;
export type BigNumber = decimal.Decimal;

Decimal.config({
  precision: 20,
  rounding: 4,
  modulo: 1,
  toExpNeg: -7,
  toExpPos: 21,
  minE: -9e15,
  maxE: 9e15,
  crypto: undefined
});

// const $PI = '3.';
// const $E = '2.';



export const pi = (new BigNumber(-1) as any).acos(); // BigNumber.acos(-1);
export const twoPiSqrt = pi.times(2).sqrt();

export const zero = new BigNumber(0);
export const one = new BigNumber(1);
export const e = Decimal.exp(1);

const P = BigNumber.prototype;

P.empty = () => zero;

const _valueOf = P.valueOf;
P.valueOf = P.toJSON = function() {
  return Number(_valueOf.apply(this));
};

/* BigNumber.prototype.toJSON = function() {  // todo: preserve precision in JSON output
  return { '@@BigNumber': _valueOf.apply(this) };
}; */

BigNumber.prototype.inspect = function() {
  return this.toString();
};

BigNumber.prototype.fromJSON = function(json) {
  return new BigNumber(json);
};

BigNumber.prototype.gamma = function() {
  // more tests
  // The Lanczos approximation
  // https://en.wikipedia.org/wiki/Lanczos_approximation
  // G(z+1) = sqrt(2*pi)*(z+g+1/2)^(z+1/2)*exp(-(z+g+1/2))*Ag(z)
  // Ag(z) = c0 + sum(k=1..N, ck/(z+k))
  // todo: Memoization?

  if (this.lt(0.5)) {
    return pi.div(
      this.times(pi)
        .sin()
        .times((one.minus(this) as any).gamma())
    );
  }

  const z = this.minus(1);
  let agz: decimal.Decimal | number = c[0];

  for (let k = 1; k < c.length; ++k) {
    const den = z.plus(k);
    if (den.isZero()) {
      agz = c[k] < 0 ? -Infinity : Infinity;
    } else {
      agz = new BigNumber(c[k]).div(den).plus(agz); //  Ag += c(k)/(z+k)
    }
  }

  const t = z.plus(g + 0.5); // z+g+1/2

  return twoPiSqrt // sqrt(2*PI)
    .times(t.pow(z.plus(0.5))) //  *(z+g+1/2)^(z+0.5)
    .times(t.neg().exp()) //  *exp(-(z+g+1/2))
    .times(agz); //  *Ag(z)
};

BigNumber.prototype.nemesClosed = function() {
  // http://www.ebyte.it/library/downloads/2007_MTH_Nemes_GammaFunction.pdf

  const z = this;

  const a = z.div(e).pow(z);
  const b = twoPiSqrt.div(z.sqrt());
  const c = one.plus(one.div(z.times(z).times(15))).pow(z.times(5).div(4));

  return a.times(b).times(c);
};

BigNumber.prototype.spouge = function() {
  const z = this;

  if (z.isNeg()) {
    return Number('0/0');
  }

  let x = new BigNumber(c[0]);
  for (let i = c.length - 1; i > 0; --i) {
    const den = z.plus(i);
    const num = new BigNumber(c[i]);
    x = x.plus(num.div(den));
  }

  const a = z.plus(1 / 2);
  const t = a.plus(g);

  const b = pi
    .times(2)
    .ln()
    .div(2)
    .plus(a.times(t.ln()))
    .minus(t)
    .plus(Math.log(+x))
    .minus(z.ln());

  return b.exp();
};

typed.addType({
  name: 'BigNumber',
  test: x => {
    return x instanceof BigNumber;
  }
});

typed.addConversion({
  from: 'number',
  to: 'BigNumber',
  convert: x => {
    return new BigNumber(x);
  }
});

typed.addConversion({
  from: 'BigNumber',
  to: 'number',
  convert: x => {
    return x.valueOf();
  }
});
