...
import { Milton, reference, ansiColors, arrayDecender, objectDecender, jsValues, jsonValues, indent, breakLength, COLORIZE_OPTIONS  } from '@hypercubed/milton';
...
const toString = () => (s: any, _p: any, v: any) => {
  if (v instanceof Word || v instanceof Key || v instanceof Decimal || v instanceof Complex || v instanceof Sentence) {
    return v.toString();
  }
  return s;
};
...
function pretty(_: Milton) {
  _.add(reference);

  _.add(arrayDecender, { comma: false });
...
export const ffPrettyPrint = {
  ...
  stringify: color.stringify.bind(color),
  ...
  formatTrace({ stack, queue, currentAction }, max = ffPrettyPrint.consoleWidth): string {
    ...
    const stackString = ffPrettyPrint.color(stack).replace(/\n/, ' ');
    const lastActionString = is.undefined(currentAction)
      ? ''
      : ffPrettyPrint.color(currentAction).replace(/\n/, ' ');
    const queueString = ffPrettyPrint.color(queue).replace(/\n/, ' ');
